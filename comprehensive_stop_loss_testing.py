#!/usr/bin/env python3
"""
Комплексное тестирование всех стратегий с детальным анализом стоп-лоссов
"""
...
class ComprehensiveStopLossTester:
    """Комплексный тестер с детальным анализом стоп-лоссов"""
    ...
    def execute_stop_order(self, order: StopOrder, current_price: float):
        """Выполнение стоп-ордера с детальной статистикой"""
        try:
            ...
            # Обновляем статистику стоп-лоссов
            if order.order_type == 'stop_loss':
                self.stop_loss_stats['total_stop_losses'] += 1
                self.stop_loss_stats['stop_loss_pnl'] += pnl
                self.stop_loss_stats['avg_stop_loss_pct'] = (
                    self.stop_loss_stats['avg_stop_loss_pct'] * (self.stop_loss_stats['total_stop_losses'] - 1) + 
                    pnl_pct
                ) / self.stop_loss_stats['total_stop_losses']
                
            elif order.order_type == 'take_profit':
                self.stop_loss_stats['total_take_profits'] += 1
                self.stop_loss_stats['take_profit_pnl'] += pnl
                self.stop_loss_stats['avg_take_profit_pct'] = (
                    self.stop_loss_stats['avg_take_profit_pct'] * (self.stop_loss_stats['total_take_profits'] - 1) + 
                    pnl_pct
                ) / self.stop_loss_stats['total_take_profits']
            ...
            logger.info(f"Выполнен {order.order_type} для {order.symbol}: PnL={pnl:.2f}₽ ({pnl_pct:.2f}%)")
    ...
    def test_conservative_strategy_with_stops(self, symbol: str, data: pd.DataFrame):
        """Консервативная стратегия с детальным анализом стоп-лоссов"""
        try:
            logger.info(f"Тестирование консервативной стратегии с стоп-лоссами для {symbol}")
            ...
            return self._calculate_detailed_metrics(symbol, "Conservative_StopLoss")
            
        except Exception as e:
            logger.error(f"Ошибка в консервативной стратегии с стоп-лоссами для {symbol}: {e}")
            return None
    
    def test_momentum_strategy_with_stops(self, symbol: str, data: pd.DataFrame):
        """Моментум стратегия с детальным анализом стоп-лоссов"""
        try:
            logger.info(f"Тестирование моментум стратегии с стоп-лоссами для {symbol}")
            ...
            return self._calculate_detailed_metrics(symbol, "Momentum_StopLoss")
            
        except Exception as e:
            logger.error(f"Ошибка в моментум стратегии с стоп-лоссами для {symbol}: {e}")
            return None
    
    def test_aggressive_strategy_with_stops(self, symbol: str, data: pd.DataFrame):
        """Агрессивная стратегия с детальным анализом стоп-лоссов"""
        try:
            logger.info(f"Тестирование агрессивной стратегии с стоп-лоссами для {symbol}")
            ...
            return self._calculate_detailed_metrics(symbol, "Aggressive_StopLoss")
            
        except Exception as e:
            logger.error(f"Ошибка в агрессивной стратегии с стоп-лоссами для {symbol}: {e}")
            return None
    
    def test_ml_strategy_with_stops(self, symbol: str, data: pd.DataFrame):
        """ML стратегия с детальным анализом стоп-лоссов"""
        try:
            if len(data) < 50:
                return None
                
            logger.info(f"Тестирование ML стратегии с стоп-лоссами для {symbol}")
            ...
            return self._calculate_detailed_metrics(symbol, "ML_StopLoss")
            
        except Exception as e:
            logger.error(f"Ошибка в ML стратегии с стоп-лоссами для {symbol}: {e}")
            return None
    ...
    def _calculate_detailed_metrics(self, symbol: str, strategy_name: str):
        """Расчет детальных метрик с анализом стоп-лоссов"""
        try:
            ...
            # Детальная статистика стоп-лоссов
            if self.trades:
                self.stop_loss_stats['stop_loss_frequency'] = len(stop_loss_trades) / len(self.trades) * 100
                self.stop_loss_stats['take_profit_frequency'] = len(take_profit_trades) / len(self.trades) * 100
            
            # Средние значения
            avg_stop_loss_pct = np.mean([t['return_pct'] for t in stop_loss_trades]) if stop_loss_trades else 0
            avg_take_profit_pct = np.mean([t['return_pct'] for t in take_profit_trades]) if take_profit_trades else 0
            ...
            return {
                ...
                'stop_loss_stats': {
                    'total_stop_losses': self.stop_loss_stats['total_stop_losses'],
                    'total_take_profits': self.stop_loss_stats['total_take_profits'],
                    'stop_loss_pnl': float(self.stop_loss_stats['stop_loss_pnl']),
                    'take_profit_pnl': float(self.stop_loss_stats['take_profit_pnl']),
                    'avg_stop_loss_pct': float(avg_stop_loss_pct),
                    'avg_take_profit_pct': float(avg_take_profit_pct),
                    'stop_loss_frequency': float(self.stop_loss_stats['stop_loss_frequency']),
                    'take_profit_frequency': float(self.stop_loss_stats['take_profit_frequency']),
    ...
    def run_comprehensive_stop_loss_testing(self):
        """Запуск комплексного тестирования с анализом стоп-лоссов"""
        logger.info("🛡️ КОМПЛЕКСНОЕ ТЕСТИРОВАНИЕ С ДЕТАЛЬНЫМ АНАЛИЗОМ СТОП-ЛОССОВ")
        logger.info("=" * 80)
        ...
        for data_key, data in market_data.items():
            ...
            logger.info(f"\n📈 Тестирование {data_key} с детальным анализом стоп-лоссов...")
            
            results = {}
            
            # Тестируем все стратегии
            strategies = [
                ('Conservative_StopLoss', self.test_conservative_strategy_with_stops),
                ('Momentum_StopLoss', self.test_momentum_strategy_with_stops),
                ('Aggressive_StopLoss', self.test_aggressive_strategy_with_stops),
                ('ML_StopLoss', self.test_ml_strategy_with_stops)
            ]
            ...
            for strategy_name, strategy_func in strategies:
                ...
                if result:
                    ...
                    stats = result['stop_loss_stats']
                    logger.info(f"    ✅ {result['monthly_return']:.2f}% в месяц, Sharpe: {result['sharpe_ratio']:.2f}")
                    logger.info(f"    🛡️ Стоп-лоссов: {stats['total_stop_losses']}, Тейк-профитов: {stats['total_take_profits']}")
                    logger.info(f"    📊 Частота стоп-лоссов: {stats['stop_loss_frequency']:.1f}%, Средний убыток: {stats['avg_stop_loss_pct']:.2f}%")
                    logger.info(f"    📈 Частота тейк-профитов: {stats['take_profit_frequency']:.1f}%, Средняя прибыль: {stats['avg_take_profit_pct']:.2f}%")
                else:
    ...
    def _save_comprehensive_results(self, results):
        """Сохранение комплексных результатов"""
        output_dir = 'backtesting/results'
        os.makedirs(output_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        json_filename = os.path.join(output_dir, f'comprehensive_stop_loss_test_{timestamp}.json')
        txt_filename = os.path.join(output_dir, f'comprehensive_stop_loss_test_{timestamp}.txt')
        ...
        with open(txt_filename, 'w', encoding='utf-8') as f:
            f.write("🛡️ КОМПЛЕКСНЫЙ ОТЧЕТ ПО ТЕСТИРОВАНИЮ СТОП-ЛОССОВ\n")
            f.write("=" * 80 + "\n\n")
            ...
            for data_key, data_results in results.items():
                ...
                for strategy_name, strategy_result in data_results.items():
                    ...
                    f.write(f"    • Всего сделок: {strategy_result['total_trades']}\n")
                    
                    stats = strategy_result['stop_loss_stats']
                    f.write(f"  🛡️ СТАТИСТИКА СТОП-ЛОССОВ:\n")
                    f.write(f"    • Стоп-лоссов: {stats['total_stop_losses']}\n")
                    f.write(f"    • Тейк-профитов: {stats['total_take_profits']}\n")
                    f.write(f"    • Частота стоп-лоссов: {stats['stop_loss_frequency']:.1f}%\n")
                    f.write(f"    • Частота тейк-профитов: {stats['take_profit_frequency']:.1f}%\n")
                    f.write(f"    • Средний убыток по стоп-лоссу: {stats['avg_stop_loss_pct']:.2f}%\n")
                    f.write(f"    • Средняя прибыль по тейк-профиту: {stats['avg_take_profit_pct']:.2f}%\n")
                    f.write(f"    • Среднее время удержания: {stats['avg_days_held']:.1f} дней\n")
                    f.write(f"    • Общий PnL стоп-лоссов: {stats['stop_loss_pnl']:.2f}₽\n")
                    f.write(f"    • Общий PnL тейк-профитов: {stats['take_profit_pnl']:.2f}₽\n")
        
        logger.info(f"✅ Результаты сохранены в {json_filename} и {txt_filename}")
    
    def _print_comprehensive_summary(self, results):
        """Вывод комплексной сводки"""
        logger.info(f"\n🛡️ КОМПЛЕКСНАЯ СВОДКА ПО СТОП-ЛОССАМ:")
        ...
        for data_key, data_results in results.items():
            for strategy_name, strategy_result in data_results.items():
                all_strategy_results.append({
                    ...
                    'stop_loss_stats': strategy_result['stop_loss_stats']
                })
        
        # Сортируем по доходности
        all_strategy_results.sort(key=lambda x: x['monthly_return'], reverse=True)
        
        logger.info(f"\n🏆 ТОП-10 РЕЗУЛЬТАТОВ С ДЕТАЛЬНЫМ АНАЛИЗОМ СТОП-ЛОССОВ:")
        for i, result in enumerate(all_strategy_results[:10]):
            ...
            logger.info(f"      Стоп-лоссов: {stats['total_stop_losses']} ({stats['stop_loss_frequency']:.1f}%), Тейк-профитов: {stats['total_take_profits']} ({stats['take_profit_frequency']:.1f}%)")
            logger.info(f"      Средний убыток: {stats['avg_stop_loss_pct']:.2f}%, Средняя прибыль: {stats['avg_take_profit_pct']:.2f}%")

def main():
    """Основная функция"""
    tester = ComprehensiveStopLossTester()
    results = tester.run_comprehensive_stop_loss_testing()
    
    if results:
        logger.info(f"\n🛡️ КОМПЛЕКСНОЕ ТЕСТИРОВАНИЕ СТОП-ЛОССОВ ЗАВЕРШЕНО УСПЕШНО!")
    else:
